...
format:
	...
	autopep8 --in-place --aggressive --experimental --max-line-length $(MAX_LINE) --recursive compression/
	...
	docformatter --in-place --wrap-summaries $(MAX_LINE) --wrap-descriptions $(MAX_LINE) \
		--pre-summary-newline --recursive compression/
	docformatter --in-place --wrap-summaries $(MAX_LINE) --wrap-descriptions $(MAX_LINE) \
		--pre-summary-newline --recursive test/

style: .python-check
	@echo "Check if 'compression' folder matches PEP-8 and PEP-257 guides"
	pydocstyle compression/
	pycodestyle compression/ --max-line-length=$(MAX_LINE)
...
test: clean .python-check
	coverage run --source=compression --omit=*"__init__.py" -m unittest -v
